...
{
    m_swapchains.insert(output, QSharedPointer<DumbSwapchain>::create(m_gpu, output->pixelSize(), DRM_FORMAT_XRGB8888));
    connect(output, &DrmOutput::currentModeChanged, this,
        [output, this] {
            m_swapchains[output] = QSharedPointer<DumbSwapchain>::create(m_gpu, output->pixelSize(), DRM_FORMAT_XRGB8888);
        }
    );
}
